...
def analyze_question_word(question_word, pos):
    ...
    if question_word in grammar["question words"]:
        ...
        logging.error('unknown question word: '+question_word)
...
def analyze_possessive_np(poss, np_list):
    ...
    for word in np_list[1:]:
        if word in grammar['categories']:
            ...
            logging.error('unknown category for possessive: ' + str(np_list))
...
def analyze_pronoun(pronoun, speaker):
    ...
    if pronoun in grammar['possessive']:
        ...
        logging.error('unknown pronoun: '+pronoun)
    ...
    if verb in grammar['to be']:
        ...
        if verb_lemma in grammar['verbs']:
           morphology['predicate'] = verb_lemma+'s'
            ...
            logging.error('unkown verb: '+verb+', lemma: '+verb_lemma)
...
def analyze_wh_question(words, speaker, response_type, viewed_object):
    ...
    if words[1].strip() in grammar['to be']:
        ...
        logging.error("I seem to have misunderstood the word "+words[1].strip()+" in your question")
    ...
    if third_pos in ['PRP$', 'NN','PRP']:
        ...
        logging.error("This word "+third_word+" is surprising me")

    logging.info('analysis of wh-question produced this rdf: '+ rdf)
    return rdf
...
def analyze_verb_question(words, speaker, viewed_objects):
    ...
    if not len(np_info):
        logging.error('issue  with extracting noun phrase:' + str(np))
