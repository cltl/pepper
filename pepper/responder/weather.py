...
class WeatherMoods(Responder):
    def __init__(self):
        super(WeatherMoods, self).__init__()
        self._api_key = config.TOKENS["Open_Weather"]
    ...
    def respond(self, utterance, app):
        ...
        for cue in WEATHER_INTEREST:
            # if utterance.transcript.lower().startswith(cue) or 'weather' in utterance.transcript.lower():
            if utterance.transcript.lower().startswith("weather status"):
                location = "Amsterdam"

                weather = self.get_weather(self._api_key, location)

                if math.trunc(weather['main']['temp_min']) <= 7:
                    return 1, lambda: app.say("Sometimes when it's this cold I feel a bit down")

                if math.trunc(weather['main']['temp_min']) >= 20:
                    return 1, lambda: app.say("It's really warm outside and I am feeling fine!")





# class OtherResponses(Responder):
#     def __init__(self):
#         super(WeatherResponder, self).__init__()
#         self._api_key = config.TOKENS["Open_Weather"]
#
...
# Return of main weatherResponder
# return 1, lambda: app.say("The weather in {} is {}, min temperature is {} and max temperature is {}"
                #         .format(location, weather['weather'][0]['description'],
